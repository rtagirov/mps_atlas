 subroutine calcross
    ...
    implicit none
      ...
      logical  readRoss
      real(kind=8), allocatable:: tttab(:), tptab(:)
      real(kind=8), allocatable :: tempross(:,:)
 
      real(kind=8) ::  tv, pv, co1, co2, co3, co4, y
...
!----
     ...
     if (readRoss) then 
       open(unit = 2, file='./INPUT/ross_table.dat', form='formatted', status='old')
       read(2,*) numtpoints, numppoints

!--- allocate temporary and 
       allocate(tempross(numtpoints, numppoints))
       allocate(tttab(numtpoints) )
       allocate(tptab(numppoints)) 
       allocate(tabrosskap(numt, numpres))
      
       read(2,*) (tttab(i), i = 1 , numtpoints)
       read(2,*) (tptab(i), i = 1, numppoints)
       do i = 1, numtpoints
         read(2,*) (tempross(i,j), j = 1, numppoints )
       end do
       close(unit=2)
!    -    interpolate read rosstable onto the T-P grid that the ODF has
       it = 2
       ip = 2 
       do i = 1, numt
         ...
         do j = 1, numpres
            ...
            tl = min(max(tabt(i) , tttab(1)), tttab(numtpoints) )
!
            do while (tttab(it) .le. tl .and. it .lt. numtpoints)
               it = it + 1
            end do
!
            plog = min(max(tabp(j), tptab(1)), tptab(numppoints))
!
            do while (tptab(ip) .le. plog .and. ip .lt. numppoints)
               ip = ip + 1
            end do

!
            x      = (tl   - tttab(it-1)) / (tttab(it) - tttab(it-1))
            y      = (plog - tptab(ip-1)) / (tptab(ip) - tptab(ip-1))
            co1 = (1.0d0 - x) * (1.0d0 - y) 
            co2 = (1.0d0 - x) * y 
            co3 = x * (1.0d0 - y) 
            co4 = x * y 
            ...
            tabrosskap(i,j) = co1*tempross(it-1, ip-1) + co2*tempross( it-1, ip) + &
     &             co3 *tempross(it, ip-1) +co4*tempross(it,ip)   
         end do 
       end do 
       open(unit=2, file='outross.dat', form='formatted') 
       do i = 1, numt
         write(2,*) (tabrosskap(i, j), j = 1, numpres)
       end do 
       close(unit=2) 
  

!
       print*, 'finished interpolating read Ross onto t-p grid of ODF'
       deallocate(tttab)
       deallocate(tptab)
       deallocate(tempross)
